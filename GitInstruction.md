...
Чтобы клонировать удаленный репозиторий копируем его ссылку и вводим команду:
```
git clone <URL- ссылка>
```
# 8. Pull request
Чтобы поучаствовать и внести свои изменения в чужой проект, необходимо:
* Скопировать его на свой аккаунт `GitHub`, для этого нажимаем `Fork`(отвлетвление репозитория).
* Делаем git clone версии этого репозитория со своего аккаунта
* Создаем новую ветку и в ней вносим свои изменения
* Связываем новую ветку с удаленной для отслеживания, набираем команду:
```
git push -u <имя удаленного репозитория><имя ветки>
```
* Создаем коммиты своих изменений
* Оправляем изменения на свою версию репозитория
* На `GitHub` переключаемся с ветку `main` на новую
